...
在组件的身上初始化一个currentIndex，随着鼠标的在一级分类上移动，通过监听鼠标的事件获取一级分类的index，使currentIndex ==
index ,并使用class属性绑定，动态的改变一级分类的背景颜色。
...
## 22.监听路由的变化再次发送请求获取数据

search模块的需要的服务器数据，我们已经请求到了，而且存储在Vuex仓库当中，而且有些数组数据，我们已经通过了Getters进行简化。
切记：getters简化数据而生。
商品列表、平台售卖属性已经动态数据
...
// 监听组件实例身上的属性的属性值变化
watch: {
// 监听data下面的route路由信息的变化，再次发起请求，搜素数据
...
## 24.面包屑处理关键字
场景：当面包屑的关键字清除之后，需要让兄弟组件的input框中清除，涉及到兄弟组件通信。
- props:父子组件通信
- 自定义事件：子传父
- Vuex：仓库数据
- 插槽：父子
- pubsub-js
- $bus:全局事件总线，全局事件总线是一种组件间通信的方式，适用于任意组件间通信。
